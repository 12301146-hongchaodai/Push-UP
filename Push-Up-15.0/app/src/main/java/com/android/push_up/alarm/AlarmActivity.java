...
public class AlarmActivity extends ActionBarActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        alarmRecord.setOnTouchListener(new View.OnTouchListener() {
            Drawable background = findViewById(R.id.alarmRecord).getBackground();
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){//按下事件
                    alarmRecord.setBackgroundColor(0x10000000);
                    return true;
                }
                else if(event.getAction() == MotionEvent.ACTION_CANCEL){
                    alarmRecord.setBackgroundDrawable(background);
                    return false;
                }
