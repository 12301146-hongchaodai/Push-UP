        ...
        btnCount.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                pop_btnConfirm.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        //数据保存代码
                        //判断当天是否保存了5组数据，即检查计划完成情况
                        DataToSQLite dataToSQLiteQuery = new DataToSQLite(CountActivity.this);

                        SQLiteDatabase dbRead = dataToSQLiteQuery.getReadableDatabase();
                        Cursor cursor = dbRead.query(
                                "data_count",
                                new String[]{"username","time"},
                                "username=? and time=?",
                                new String[]{sharedPreferencesName.getString("username",null),new DateFormat().format("yyyy/MM/dd", Calendar.getInstance(Locale.CHINA)).toString()},
                                null,
                                null,
                                null,
                                null);
                        ...
                        while(cursor.moveToNext()){
                            ...
                            recordList.add(dbTime);
                        }
                        dbRead.close();
                        if(recordList.size() > 4){
                            new AlertDialog.Builder(CountActivity.this).setTitle("提示").setMessage("今天的训练已经结束").setPositiveButton("确认",null).show();
                        }
                        else{
                            //获取当前计数数目
